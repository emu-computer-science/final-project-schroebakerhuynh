...
//***********************************METHODS********************************************
	...
	public void release() {
		...
		String position = "";
		boolean found = false;                        //marker to identify if the player was found int he database
		for (int i = 0; i <roster.size(); i++) {         //For each player in the database, try to match the name
			if (name.equals(roster.get(i).getPlayerName()));{       //Found the player with the desired name
				found = true;
				roster.get(i).setFantasyTeam("Free Agents");
				roster.remove(i);
			}//end if statement
		}//end for loop
	}//end release method
